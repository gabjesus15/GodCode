 ...
 *
 * `companySlug` decide dónde se guarda: el provider persiste el carrito por negocio
 * (`tenant_cart_storage_<slug>`) y lo recarga al montar el menú, así que escribirlo en
 * otra clave haría que el menú lo pisara con el carrito vacío.
 */
export function useRepeatOrder(companySlug?: string) {
	...
	const repeatOrder = useCallback(
		async (order: MenuAccountOrder): Promise<boolean> => {
			...
			if (slug) {
				// Misma clave que usa CartProvider; se rehidrata antes de escribir para no
				// perder lo demás que guarda ese carrito (p. ej. extras globales).
				persistApi?.setOptions?.({ name: `tenant_cart_storage_${slug}` });
